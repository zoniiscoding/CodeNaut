...
def test_model_load_retries_after_transient_failure_and_becomes_ready() -> None:
    model = FakeModel(load_failure_count=2)
    settings = make_settings(model_load_max_attempts=3, model_load_retry_base_seconds=0.01)
    with TestClient(create_app(settings=settings, model=model)) as client:
        deadline = time.monotonic() + 2
        response = client.get("/health/ready", headers=AUTH)
...
def test_model_load_exhausts_retries_then_fails_closed_and_reports_unhealthy() -> None:
    model = FakeModel(load_failure=True)
    settings = make_settings(model_load_max_attempts=3, model_load_retry_base_seconds=0.01)
    with TestClient(create_app(settings=settings, model=model)) as client:
        deadline = time.monotonic() + 2
        response = client.get("/health/ready", headers=AUTH)
        while time.monotonic() < deadline and response.json()["status"] == "loading":
            time.sleep(0.01)
            response = client.get("/health/ready", headers=AUTH)
        liveness = client.get("/health/live")
    assert response.status_code == 503
    ...
    assert liveness.json()["status"] == "failed"


def test_liveness_stays_ok_while_a_retry_is_pending() -> None:
    model = FakeModel(load_failure_count=1)
    settings = make_settings(model_load_max_attempts=3, model_load_retry_base_seconds=1)
    with TestClient(create_app(settings=settings, model=model)) as client:
        time.sleep(0.05)
