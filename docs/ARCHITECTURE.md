...
Configuration is validated before either app is constructed. Production additionally requires JSON logging, disabled API docs, explicit trusted hosts, origin-only HTTPS CORS/frontend values, exact HTTPS OAuth callback paths on trusted hosts, explicit PostgreSQL TLS, credentialed non-local PostgreSQL, authenticated TLS Redis, HTTPS authenticated Qdrant, HTTPS private embeddings, and one of the exact reviewed OpenAI/Gemini LLM endpoints with a non-placeholder credential. An absolute Git executable, PEM-shaped GitHub App material, non-placeholder critical secrets, and complete Google settings when enabled are mandatory. The embedding service requires an absolute cache and local-files-only model loading in production. Clone, discovery, parser, embedding, Qdrant, question, evidence, LLM, agent call/time/result/history/output, heartbeat, stream, concurrency, and retry bounds are validated. Secrets are excluded from settings representations and allowlisted startup summaries.
...
15. Temporary clones are mode `0700`, are removed on every exit, and deletion errors become safe retryable failures. Successful removal is verified before an index can activate or an already-current clone job can become terminal; a failed cleanup can therefore never be reported as successful activation.

Public repository rows and Qdrant indexes may be shared, but authorization never is. Each browser user needs an independent `user_repositories` membership. Listings may use the bounded visibility cache, while repository reads, refreshes, and every question/tool disclosure bypass it and revalidate the exact GitHub identity as currently public. A public-to-private/not-found/identity transition revokes the row and fails closed before additional evidence is sent or returned.
...
9. The service repeats authorization and active-build validation before tools, before previously retrieved evidence is sent back to the provider, and after external work. Public repositories force a fresh GitHub visibility check at those boundaries. A changed version, public/private transition, removed public membership, or revoked repository/installation fails closed.
...
## Milestone 12 deployment architecture

The checked-in deployment topology now has explicit provider boundaries. Vercel builds only `frontend/`; Railway builds `backend/` twice as separately configured API and worker services and `embedding_service/` once. Both Python images use the same immutable Python 3.13.14 multi-platform base digest and non-root users. The API is the only public Railway service. Worker and embeddings have no public domain; their internal HTTP/Redis addresses must be exact explicit-port `*.railway.internal` names on Railway's encrypted private mesh.

`SERVICE_ROLE` narrows process secrets. The API owns OAuth/OIDC, webhooks, browser sessions, questions, and the LLM key. The worker owns the GitHub App private key and repository/data-plane credentials needed for indexing but receives no OAuth client secret, webhook secret, session/token-hash secrets, Google secret, or LLM key. The embedding service owns only its shared bearer and immutable local model. Alembic has a fourth, short-lived authority: a direct least-privilege `MIGRATION_DATABASE_URL`, independent of application configuration.

Vercel validates the exact production API URL at build time and derives `connect-src` from its origin. Railway API pre-deploy applies migrations before health-gated rollout; the worker handles SIGTERM/SIGINT by stopping new deliveries while any claimed durable job drains. PostgreSQL leases/heartbeats remain the crash-recovery authority. The manual production workflow accepts only the current full `main` SHA with green CI and deploys embeddings, worker, API/migration, then frontend. This is implemented source architecture, not evidence that provider resources exist; see `DEPLOYMENT_M12.md`.
