...
## D-075 — Trust plain HTTP to the embedding service over literal loopback in production

- **Date:** 2026-08-10
- **Status:** Accepted
- **Decision:** Extend the existing production `EMBEDDING_SERVICE_URL` check (HTTPS, or plain HTTP only to a named `*.railway.internal` host) to also accept plain HTTP to a literal loopback address (`127.0.0.1`, `localhost`, `::1`) with an explicit port. No other hostname form is accepted unencrypted.
- **Rationale:** A single-host deployment (API, worker, and embedding service as sibling containers on one machine, run with `network_mode: host`) can reach the embedding service only via loopback, which structurally never leaves the host — the same trust property that justified the Railway private-network carve-out, just enforced by the kernel's loopback interface instead of Railway's WireGuard mesh. A Docker bridge-network service name (e.g. `embedding-service`) does not carry this guarantee, since any container sharing that bridge network could impersonate it; only literal loopback is trusted here.
- **Consequence:** A production deployment that puts the embedding service behind a routable hostname (Docker bridge DNS, a LAN address, anything not Railway or loopback) must still use HTTPS. This carve-out widens `EMBEDDING_SERVICE_URL` only; `DATABASE_URL`, `REDIS_URL`, and `QDRANT_URL` keep their existing TLS-or-Railway-private requirements unchanged.
