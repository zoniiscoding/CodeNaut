...
async def _load_and_warm(
    ...
    for attempt in range(1, settings.model_load_max_attempts + 1):
        try:
            ...
            logger.error(  # noqa: TRY400 -- traceback can reveal model/cache internals
                ...
                attempt=attempt,
                max_attempts=settings.model_load_max_attempts,
                error_type=type(error).__name__,
            )
            if attempt == settings.model_load_max_attempts:
                runtime.status = "failed"
                return
            await _model_load_backoff(attempt, settings.model_load_retry_base_seconds)
            continue
...
async def _model_load_backoff(attempt: int, base_seconds: float) -> None:
...
def _register_health_routes(
    ...
    async def liveness() -> LivenessResponse | JSONResponse:
        if runtime.status == "failed":
            return JSONResponse(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                content=LivenessResponse(status="failed").model_dump(),
            )
